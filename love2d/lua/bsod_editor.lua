...
local function parseFileLine(text)
  ...
  -- or: [string "lua/foo.lua"]:42: message
  -- or: @lua/foo.lua:42: message
  ...
  if not path then
    -- Lua traceback chunk-name form
    path, lineStr = text:match('%[string "(@?[%w%.%_/%-\\]+%.lua)"%]:(%d+)')
  end
  if not path then
    path, lineStr = text:match("%[string '(@?[%w%.%_/%-\\]+%.lua)'%]:(%d+)")
  end
  if not path then
    -- Try anywhere in the string (for stack trace lines with prefixes)
    path, lineStr = text:match("(@?[%w%.%_/%-\\]+%.lua):(%d+)")
  end
...
local function findCrashLocation(message, stack)
  ...
  if stack then
    for traceLine in stack:gmatch("[^\n]+") do
      -- Skip [C] frames. Keep [string "..."] frames because they may carry
      -- real file paths (e.g. [string "lua/masks.lua"]:123).
      if not traceLine:match("%[C%]") then
        path, line = parseFileLine(traceLine)
        if path and path ~= "boot.lua" then return path, line end
      end
    end
  end
