...
function Mandala.update(state, dt, props, w, h, mouse)
  ...
  for _, slice in ipairs(state.slices) do
    ...
    slice.innerRadius = math.max(2, slice.innerRadius + slice.radialDrift * dt * 10)
    slice.outerRadius = math.max(slice.innerRadius + 1, slice.outerRadius + slice.radialDrift * dt * 10)

    if slice.age < slice.maxAge then
      local life = 1 - slice.age / slice.maxAge
      slice.alpha = math.max(0, math.min(1, slice.alpha * (1 - dt * 0.35) * (0.8 + life * 0.25)))
