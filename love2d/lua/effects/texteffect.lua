...
local function drawEditorIllustration(state, props, w, h)
  ...
  if codeFont:getWidth(typedLine) > maxTypedWidth then
    local lineChars = toChars(typedLine)
    while #lineChars > 6 and codeFont:getWidth(charsToString(lineChars)) > maxTypedWidth do
      lineChars[#lineChars] = nil
    end
    lineChars[#lineChars + 1] = "…"
    typedLine = charsToString(lineChars)
  end
