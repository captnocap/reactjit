// Detect overlap between the TileLayer CodeBlock and the TileSourceDemo
// in the Geo story. The long URL strings in the code block cause it to
// overflow into the adjacent Half at certain breakpoints.
//
// Run: cd storybook && rjit test tests/geo-tilelayer-overlap.test.ts
...
test('Geo story: detect tilelayer section overlap at each breakpoint', async function () {
  await page.wait(3);

  // Geo is at index 30 in the stories array
  ...
  for (var bi = 0; bi < BREAKPOINTS.length; bi++) {
    ...
    // 2. Text audit — all rules (escape, overlap, codeblock overlap, truncation)
    ...
    await page.screenshot('/tmp/geo-story-' + bp.name + '.png');
  }

  // Write violations to file for easy reading
  ...
  await _bridge.rpc('test:writeFile', {
    path: '/tmp/geo-story-violations.txt',
    content: allViolations.length + ' violations:\n' + summary,
  });

  if (allViolations.length > 0) {
    throw new Error(allViolations.length + ' violations found. See /tmp/geo-story-violations.txt');
  }
});
