...
export async function testCommand(args) {
  ...
  // Prefer xvfb-run for headless CI environments (skip with --visible)
  const visible = args.includes('--visible');
  const xvfbRun = visible ? null : findExecutable('xvfb-run');
  ...
  return new Promise((done) => {
    ...
    // In verbose mode, show all stdout lines (not just TEST_ prefixed)
    if (args.includes('--verbose') || args.includes('-v')) {
      proc.stdout.on('data', (d) => {
        for (const line of d.toString().split('\n')) {
          if (!line.startsWith('TEST_')) {
            const trimmed = line.trim();
            if (trimmed) console.log(`  ${dim(trimmed)}`);
          }
        }
      });
    }
