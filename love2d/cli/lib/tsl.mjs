...
function looksLikeString(node) {
  ...
  // String() / tostring() calls always return strings
  if (ts.isCallExpression(node) && ts.isIdentifier(node.expression)) {
    if (['String', 'tostring'].includes(node.expression.text)) return true;
  }
  // Conditional expression (ternary): if either branch is string-like, result is string-like
  if (node.kind === SK.ConditionalExpression) {
    if (looksLikeString(node.whenTrue) || looksLikeString(node.whenFalse)) return true;
  }
  ...
  if (ts.isPropertyAccessExpression(node)) {
    ...
    if (['name', 'title', 'description', 'text', 'label', 'message', 'type',
         'id', 'url', 'href', 'src', 'className', 'tag', 'key', 'value',
         'formula', 'symbol'].includes(prop)) return true;
  }
