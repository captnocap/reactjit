...
export function initEventDispatching(bridge: Subscribable): void {
  ...
  bridge.subscribe('texteditor:state', (event: LoveEvent) => {
    dispatchToTargetOnly(event, 'onTextEditorState');
  });
  ...
  if (bubblePath.length > 0) {
    ...
    if (typeof handler === 'function') {
      try {
        ...
        reportError(e, `${handlerName} for node ${event.targetId}`);
      }
    }
  }
}
...
function broadcastToAll(event: LoveEvent, handlerName: string): void {
  for (const [nodeId, handlers] of handlerRegistry) {
    const handler = handlers[handlerName];
    if (typeof handler === 'function') {
      try {
        handler(event);
      } catch (e) {
        reportError(e, `${handlerName} broadcast for node ${nodeId}`);
      }
    }
  }
}
