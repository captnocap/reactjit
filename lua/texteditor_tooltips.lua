...
tooltips["Box"] = {
  ...
  guided   = "Core container primitive. Renders as a View node. No intrinsic size — must be explicit or auto-sized by children. Row Boxes need explicit width for justifyContent to work.",
...
tooltips["Image"] = {
  ...
  guided   = "Image primitive. src is a Love2D asset path. Supports objectFit: fill/contain/cover/none. Needs explicit dimensions or auto-sizes to native image size.",
...
tooltips["justifyContent"] = {
  ...
  guided   = "justifyContent — main-axis distribution. Row Boxes NEED explicit width for this to work (Box has no intrinsic width). Values: start, center, end, space-between, space-around, space-evenly.",
...
tooltips["overflow"] = {
  beginner = "overflow controls what happens when content is bigger than its container. 'hidden' clips the overflow, 'scroll' adds scrolling, 'visible' lets it spill out.",
  guided   = "overflow — content clipping. 'hidden' enables scissor rect, 'scroll' enables Lua scroll handling, 'visible' (default) lets content exceed bounds.",
  clean    = "overflow — 'visible' | 'hidden' | 'scroll'",
}
