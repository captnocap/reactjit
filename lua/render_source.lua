...
local function buildFFmpegCmd(parsed, opts)
  ...
  elseif parsed.type == "window" then
    return string.format(
      ...
      .. "ffmpeg %s -f x11grab -framerate %d -video_size ${WIDTH}x${HEIGHT} "
      .. "-i %s+${X},${Y} -f rawvideo -pix_fmt rgba -an -sn - 2>/dev/null"
      .. "'",
      parsed.title, Q, fps, os.getenv("DISPLAY") or ":0"
    ), w, h
...
function RenderSource.updateAll()
  for nodeId, feed in pairs(feeds) do
    ...
    if feed.backend == "xshm" then
      ...
      local ok = captureXShm(feed.xshmCap, xroot, 0, 0, dest,
