...
local function computeData(props)
  local TABLE_LAYOUT = {
      ...
      return { TABLE_LAYOUT = TABLE_LAYOUT, s = s, h = h, gap = gap }
end
local function rebuildList_0(wrapperNodeId, items)
  ...
  for _i, row in ipairs(items) do
    tmpl[#tmpl + 1] =
    { type = "View", key = "li_0_" .. _i, style = { flexDirection = "row", gap = nil }, children = {
      { type = "__TEXT__", key = "li_0_e1_" .. _i, text = row.map(z => {
              if (z === 0) {
                return <Box style={{ width: s, height: h }} />
              }
              return (
                <ElementTile
                  element={z}
...
local function buildTemplate()
  return {
    { type = "View", key = "n0", style = { gap = nil }, children = {
      { type = "View", key = "n0_1_list_0", style = { gap = nil } },
      } },
  }
end
...
local function updateTree(handles, props)
  ...
  rebuildList_0(handles["n0_1_list_0"], data.TABLE_LAYOUT)
end
