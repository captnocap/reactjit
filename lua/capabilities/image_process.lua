...
local function ensureFFI()
  ...
  -- image_load / image_free may already be declared by sdl2_images.lua.
  -- pcall tolerates the duplicate; if sdl2_images hasn't loaded, this declares them.
  pcall(ffi.cdef, [[
...
if love and love.timer then
  ...
  local has_sdl, SDL = pcall(require, "lua.sdl2_gl")
  if has_sdl and SDL.SDL_GetPerformanceCounter then
    local freq = tonumber(SDL.SDL_GetPerformanceFrequency())
    clock = function()
      return tonumber(SDL.SDL_GetPerformanceCounter()) / freq
    end
  else
