//! Generated by tsz compiler — rings [handlers.zig]
//! Source: rings.tsz

const std = @import("std");

// [origin:rings:handlers]
        ...
        while (y < @as(f32, @floatFromInt(ctx_e.height))) : (y += 1.0) {
            {
                ...
                while (x < @as(f32, @floatFromInt(ctx_e.width))) : (x += 1.0) {
                    ...
                    const ring: f32 = @sin(dist * 0.05 - t * 3.0) * 0.5 + 0.5;
                    const hue: f32 = dist * 0.01 + t * 0.2;
                    const rgb: [3]f32 = effect_ctx.EffectContext.hsvToRgb(hue, 0.8, ring);
                    ctx_e.setPixel(x, y, rgb[0], rgb[1], rgb[2], ring);
                }
            }
        }
    }
...
pub const _effect_wgsl_0: []const u8 =
    ...
    \\  out_color = vec4f(rgb.x, rgb.y, rgb.z, ring);
    \\  return out_color;
    \\}
    \\
;
