//! Generated by tsz compiler — spirograph [handlers.zig]
//! Source: spirograph.tsz

const std = @import("std");

// [origin:spirograph:handlers]
...
pub fn _effect_render_0(ctx_e: *effect_ctx.EffectContext) void {
    // TODO: // Trail decay — fade alpha
    ctx_e.fade(0.97);
    const cx: f32 = @as(f32, @floatFromInt(ctx_e.width)) * 0.5;
    const cy: f32 = @as(f32, @floatFromInt(ctx_e.height)) * 0.5;
    const scale: f32 = @as(f32, @floatFromInt(ctx_e.width)) * 0.35;
    const t: f32 = ctx_e.time;
    const R1: f32 = scale * (0.6 + @sin(t * 0.23) * 0.2 + 0.2);
    const R2: f32 = scale * (0.3 + @sin(t * 0.31) * 0.175 + 0.175);
    const d: f32 = scale * (0.2 + @sin(t * 0.41) * 0.2 + 0.2);
    {
        var step: f32 = 0.0;
        while (step < 200) : (step += 1.0) {
            const angle: f32 = t * 2.0 + step * 0.02;
            const ratio: f32 = (R1 - R2) / R2;
            const px: f32 = (R1 - R2) * @cos(angle) + d * @cos(angle * ratio);
            const py: f32 = (R1 - R2) * @sin(angle) - d * @sin(angle * ratio);
            const ix: f32 = cx + px;
            const iy: f32 = cy + py;
            const hue: f32 = t * 0.1 + step * 0.002;
            const rgb: [3]f32 = effect_ctx.EffectContext.hsvToRgb(hue, 0.9, 0.85);
            ctx_e.setPixel(ix, iy, rgb[0], rgb[1], rgb[2], 1.0);
        }
    }
}
...
pub const _effect_wgsl_0: []const u8 =
    ...
    \\  for (var step: f32 = 0; step < 200; step = step + 1.0) {
    \\    let angle = t * 2.0 + step * 0.02;
    \\    let ratio = (R1 - R2) / R2;
    \\    let px = (R1 - R2) * cos(angle) + d * cos(angle * ratio);
    \\    let py = (R1 - R2) * sin(angle) - d * sin(angle * ratio);
    \\    let ix = cx + px;
    \\    let iy = cy + py;
    \\    let hue = t * 0.1 + step * 0.002;
    \\    let rgb = hsv2rgb(hue, 0.9, 0.85);
    \\    if (abs(x - (ix)) < 0.5 && abs(y - (iy)) < 0.5) { out_color = vec4f(rgb.x, rgb.y, rgb.z, 1.0); }
    \\  }
    \\  return out_color;
    \\}
    \\
