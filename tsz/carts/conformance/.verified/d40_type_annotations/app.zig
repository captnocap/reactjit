...
fn _updateDynamicTexts() void {
    st._dyn_text_0 = std.fmt.bufPrint(&st._dyn_buf_0, "name: {s}", .{ state.getSlotString(0) }) catch "";
    nodes._arr_0[0].text = st._dyn_text_0;
    st._dyn_text_1 = std.fmt.bufPrint(&st._dyn_buf_1, "score: {d}", .{ state.getSlot(1) }) catch "";
    nodes._arr_0[1].text = st._dyn_text_1;
    st._dyn_text_2 = std.fmt.bufPrint(&st._dyn_buf_2, "{s}", .{ state.getSlotString(4) }) catch "";
    nodes._arr_10[4].text = st._dyn_text_2;
}

fn _updateConditionals() void {
    nodes._arr_0[2].style.display = if ((state.getSlot(3) ==  1)) .flex else .none;
}
...
export fn app_get_title() [*:0]const u8 { return "d40_type_annotations"; }

export fn app_state_count() usize { return 5; }
const _slot_types = [_]u8{ 3, 0, 1, 0, 3 };
