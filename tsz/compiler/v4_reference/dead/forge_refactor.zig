...
//!
//! Usage: forge-refactor build <file.tsz>
//!
//! 1. Reads .tsz source
...
// Smith JS source — generated bundle from compiler/smith_refactor/smith_LOAD_ORDER.txt
const SMITH_JS = @embedFile("smith_refactor/dist/smith.bundle.js");
...
pub fn main() !void {
    ...
    const cmd = args.next() orelse {
        std.debug.print("Usage: forge-refactor build <file.tsz>\n", .{});
        return;
    };
    ...
    if (!got_path) {
        std.debug.print("Usage: forge-refactor build [--fast] [--mod] <file.tsz>\n", .{});
        return;
    }
    ...
    // 5. Load Smith JS
    if (!smith.loadModule(SMITH_JS, "compiler/smith_refactor/dist/smith.bundle.js")) {
    ...
    if (zig_output.len > split_marker.len and std.mem.startsWith(u8, zig_output, split_marker)) {
        ...
        const dir_path = std.fmt.allocPrint(Alloc, "refactor_generated_{s}", .{stem}) catch return;
        ...
        const out_path = std.fmt.allocPrint(Alloc, "refactor_generated_{s}{s}", .{ stem, ext }) catch return;
