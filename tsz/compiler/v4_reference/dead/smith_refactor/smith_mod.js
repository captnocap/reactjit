...
function emitModBody(lines, startIdx, typeNames, depth, allVariants, retType, ctx) {
  ...
  while (i < lines.length) {
    ...
    if (ternaryMatch) {
      ...
      out += ind + '}\n';
      if (ternaryMatch[3].trim() === 'go' && isEarlyExitBranch(ternaryMatch[2])) {
    ...
    if (forMatch) {
      ...
      out += emitForLoopV2(forMatch[1], forMatch[2], forBody, typeNames, depth, extendModCtx(ctx, { narrowedVars: narrowedVars })); continue;
    }
...
function emitForLoopV2(arrExpr, itemVar, bodyLines, typeNames, depth, ctx) {
  let out = ''; const ind = '    '.repeat(depth);
  const rangeMatch = arrExpr.match(/^(\w+)\[(\d+)\.\.(\w+)\]$/);
  const baseArr = rangeMatch ? rangeMatch[1] : arrExpr;
  ...
  while (i < bodyLines.length) {
    ...
    if (i + 2 < bodyLines.length && bodyLines[i+1].text.startsWith('? ') && bodyLines[i+2].text.startsWith(': ')) {
      const cond = modTranspileForExprV2(text, baseArr, itemVar, ctx);
      const trueExpr = modTranspileForExprV2(bodyLines[i+1].text.slice(2).trim(), baseArr, itemVar, ctx);
    ...
    // Single line
    const processed = modTranspileForExprV2(text, baseArr, itemVar, ctx);
