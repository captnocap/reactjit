...
//   nodes.zig, handlers.zig, effects.zig, state.zig, maps.zig,
//   js_logic.zig, lua_logic.zig, runtime_updates.zig, app.zig
...
function splitOutput(monolith, file) {
  ...
  F['handlers.zig']        = sec.handlers || '';
  F['effects.zig']         = sec.effects || '';
  F['state.zig']           = (sec.state_manifest || '') + (sec.dyntxt || '') +
                             (sec.oa || '') + (sec.initstate || '');
  F['maps.zig']            = sec.maps || '';
  F['js_logic.zig']        = sec.jslogic || '';
  F['lua_logic.zig']       = sec.lualogic || '';
  F['runtime_updates.zig'] = (sec.updatedyn || '') + (sec.updatecond || '') +
                             (sec.updatevariants || '');
  F['app.zig']             = (sec.appinit || '') + (sec.apptick || '') +
  ...
  // nodes.zig: handler function refs → handlers.X / effects.X
  if (F['nodes.zig']) {
    F['nodes.zig'] = F['nodes.zig'].replace(/= (_handler_\w+)/g, '= handlers.$1');
    F['nodes.zig'] = F['nodes.zig'].replace(/= (_effect_render_\w+)/g, '= effects.$1');
  }
  ...
  // runtime_updates.zig: all cross-module refs
  if (F['runtime_updates.zig']) {
    var uc = F['runtime_updates.zig'];
    uc = prefixArrRefs(uc, 'nodes.');
    uc = uc.replace(/\b(_root)\b/g, 'nodes.$1');
    uc = uc.replace(/\b(_dyn_(?:buf|text)_\d+)\b/g, 'st.$1');
    uc = uc.replace(/\b(_oa\d+_\w+)\b/g, 'st.$1');
    uc = uc.replace(/\b(_setVariantHost)\b/g, 'st.$1');
    uc = uc.replace(/\b(_pool_arena)\b/g, 'maps.$1');
    uc = uc.replace(/\b(_rebuildMap\d+)\b/g, 'maps.$1');
    uc = uc.replace(/\b(_map_count_\d+)\b/g, 'maps.$1');
    uc = uc.replace(/\b(_map_pool_\d+)\b/g, 'maps.$1');
    uc = uc.replace(/\b(_initMapLuaPtrs\d+_\d+)\b/g, 'maps.$1');
    F['runtime_updates.zig'] = uc;
  }
  ...
  if (F['app.zig']) {
    ...
    ac = ac.replace(/\b_updateDynamicTexts\(/g, 'updates._updateDynamicTexts(');
    ac = ac.replace(/\b_updateConditionals\(/g, 'updates._updateConditionals(');
    ac = ac.replace(/\b_updateVariants\(/g, 'updates._updateVariants(');
    ac = ac.replace(/\bJS_LOGIC\b/g, 'js_logic.JS_LOGIC');
    ac = ac.replace(/\bLUA_LOGIC\b/g, 'lua_logic.LUA_LOGIC');
    F['app.zig'] = ac;
  }
  ...
  function mkHeader(fname) {
    ...
    if (fname !== 'js_logic.zig' && fname !== 'lua_logic.zig') h += 'const std = @import("std");\n';

    if (!fastBuild && fname !== 'js_logic.zig' && fname !== 'lua_logic.zig') {
    ...
    if (fname === 'nodes.zig') {
      if (F['nodes.zig'] && F['nodes.zig'].indexOf('handlers.') >= 0)
        h += 'const handlers = @import("handlers.zig");\n';
      if (F['nodes.zig'] && F['nodes.zig'].indexOf('effects.') >= 0)
        h += 'const effects = @import("effects.zig");\n';
    }
    if (fname === 'maps.zig') {
    ...
    if (fname === 'runtime_updates.zig') {
      if (F['runtime_updates.zig'] && F['runtime_updates.zig'].indexOf('nodes.') >= 0)
        h += 'const nodes = @import("nodes.zig");\n';
      if (F['runtime_updates.zig'] && F['runtime_updates.zig'].indexOf('st.') >= 0)
        h += 'const st = @import("state.zig");\n';
      if (F['runtime_updates.zig'] && F['runtime_updates.zig'].indexOf('maps.') >= 0)
    ...
    if (fname === 'app.zig') {
      ...
      if (F['handlers.zig'] && F['handlers.zig'].trim())
        h += 'const handlers = @import("handlers.zig");\n';
      if (F['runtime_updates.zig'] && F['runtime_updates.zig'].trim())
        h += 'const updates = @import("runtime_updates.zig");\n';
      if (F['js_logic.zig'] && F['js_logic.zig'].trim())
        h += 'const js_logic = @import("js_logic.zig");\n';
      if (F['lua_logic.zig'] && F['lua_logic.zig'].trim())
        h += 'const lua_logic = @import("lua_logic.zig");\n';
    }
