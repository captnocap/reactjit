...
function smithCheck() {
  ...
  if (lane === 'chad') {
    ...
    path.push('extractBlocks(var,state,functions,types,data,+legacyTimerCompat)');
  }
...
// IMPORTANT:
//   - Active dictionary syntax is intent-style `<name module> ... </name>`
//   - `<module name> ... </module>` is a compatibility block surface used by
//     the layout/legacy atom path in `smith/mod/*`
//   - The line-by-line transpiler below is legacy TS/JS compatibility, not the
//     dictionary-first intent-module compiler
function compileMod(source, file) {
  var moduleContract = typeof buildModuleSourceContract === 'function'
    ...
    : null;
  if (moduleContract && typeof compileModuleFromSourceContract === 'function') {
    var contractOut = compileModuleFromSourceContract(moduleContract, source, file);
    if (typeof contractOut === 'string') return contractOut;
  }

  // Compatibility block-module syntax: `<module name> ... </module>`
  if (isCompatModuleBlockSource(source)) {
    return compileModBlock(source, file);
  }

  // Active intent-module syntax currently falls through to the older
  // compatibility transpiler below until it gets its own normalized emit path.
  // Keep the distinction explicit so we do not confuse the compatibility path
  // with the dictionary contract.
  void isIntentNamedModuleSource(source);
