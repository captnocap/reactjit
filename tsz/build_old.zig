...
pub fn build(b: *std.Build) void {
    ...
    // ── Fast cart build (api.zig types + link against engine .so) ──
    {
        ...
        // extern fns in api.zig resolve against the engine .so at link time.
        ...
        cart_fast_exe.linkLibrary(core_so);
        cart_fast_exe.linkLibC();

        const cart_fast_install = b.addInstallArtifact(cart_fast_exe, .{});
        const cart_fast_step = b.step("cart-fast", "Fast cart build — api.zig types + engine .so link");
