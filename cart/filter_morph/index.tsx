...
export default function FilterMorph() {
  ...
  useEffect(() => {
    return tween(setBootK, 1, 0, 900, EASE_OUT_CUBIC);
  }, []);

  const swapMenu = (next: MenuKey) => {
    if (next === menu) return;
    tween(setListK, 0, 1, 160, EASE_IN_OUT_CUBIC, () => {
      setMenu(next);
      setSelected(null);
      tween(setListK, 1, 0, 240, EASE_OUT_CUBIC);
    });
  };
  ...
  const list = listActive ? (
    <Filter shader="pixelate" intensity={listK} style={{ width: '100%' }}>
      {listContent}
    </Filter>
  ) : listContent;
  ...
  const body = (
    <Box style={{
      ...
      <Text style={{ fontSize: 13, color: '#a89880', marginBottom: 8 }}>
        Click a tab — the list scrambles through pixelate while it swaps.
      </Text>
  ...
  return bootActive ? (
    <Filter shader="pixelate" intensity={bootK} style={{ width: '100%', height: '100%' }}>
      {body}
    </Filter>
  ) : body;
}
