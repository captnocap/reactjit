...
//
// Polls the in-memory ring (bus.recent) every 400ms and renders the last N
// events newest-first. Filters by minimum importance + type substring. The
// full record (including parent_id chains) is on disk at
// ~/.cache/reactjit/events-<sessionId>.ndjson.
//
// This cart exists for the same reason the bus exists: when something
// goes weird, you should be able to see WHAT was happening, in the right
// order, without grep-spelunking. The "two days to find the 3.5MB cliff"
// story should never repeat — open this cart, filter to imp ≥ 0.7, the
// ipc.overflow row is right there.
...
// Importance tier presets — clicking pins the lower-bound for the list.
        ...
        <Row style={{ width: '100%', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <Col style={{ gap: 4 }}>
            ...
            <Text fontSize={11} color={TEXT_DIM}>
              session {sid || '—'} · {events.length} buffered · ring caps at 4096; full log on disk
            </Text>
          </Col>
