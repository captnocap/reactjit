...
export function ProcessCircle(_props: ProcessCircleProps) {
  const width = 180;
  const height = 180;
  ...
  return (
    <Box style={{ width, height, alignItems: 'center', justifyContent: 'center' }}>
      <Graph style={{ width, height }}>
        <Graph.Path d={bgPath} stroke="#2a2a4a" strokeWidth={12} fill="none" />
        <Graph.Path d={fillPath} stroke={PALETTE.pink} strokeWidth={12} fill="none" />
      </Graph>
      <Box style={{ position: 'absolute', alignItems: 'center' }}>
        <Text fontSize={24} color={PALETTE.white} style={{ fontWeight: 'bold' }}>{Math.round(progress * 100)}%</Text>
      </Box>
    </Box>
  );
}
