...
function wrapNodes(nodes: MathNode[]): MathNode {
...
function parseAtom(source: string, start: number): ParseResult {
  ...
  if (ch === '\\') {
    ...
    if (name === 'begin') {
      const env = source[next] === '{' ? parseBalanced(source, next + 1) : { nodes: [], next };
    ...
    if (name === 'text' || name === 'mathrm' || name === 'operatorname') {
      if (source[next] === '{') {
        ...
        return { nodes: textNode(inner.text), next: inner.next };
      }
