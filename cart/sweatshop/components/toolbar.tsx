    ...
    <Col style={{ width: '100%' }}>
      ...
      >
        <Row style={{ alignItems: 'center', gap: 8, flexGrow: 1, flexBasis: 0 }}>
          ...
          <Tooltip label="Open workspace home" side="bottom">
            <Pressable
          ...
          <Tooltip label="Go to landing view" side="bottom">
            <Pressable
              onPress={props.onOpenHome}
              style={{
        ...
        <Row style={{ alignItems: 'center', gap: 8, marginLeft: 10 }}>
          ...
          <Tooltip label="Refresh workspace" side="bottom"><HeaderButton label="Refresh" meta="R" icon="refresh" compact={compact} onPress={props.onRefreshWorkspace} /></Tooltip>
          <Tooltip label="Open settings" side="bottom"><HeaderButton label="Settings" meta="S" icon="palette" compact={compact} active={props.settingsActive ? 1 : 0} onPress={props.onOpenSettings} /></Tooltip>
          <Tooltip label="Open search" side="bottom"><HeaderButton label="Search" meta="F3" icon="search" compact={compact} active={props.searchActive ? 1 : 0} onPress={props.onToggleSearch} /></Tooltip>
          <Tooltip label="Toggle terminal" side="bottom"><HeaderButton label="Terminal" meta="~" icon="terminal" compact={compact} active={props.terminalActive ? 1 : 0} onPress={props.onToggleTerminal} /></Tooltip>
          <Tooltip label="Toggle hot panel" side="bottom"><HeaderButton label="Hot" meta="H" icon="flame" compact={compact} active={props.hotActive ? 1 : 0} onPress={props.onToggleHot} /></Tooltip>
          ...
          <Tooltip label="Open command palette" side="bottom"><HeaderButton label="Palette" meta="P" icon="command" compact={compact} active={props.paletteActive ? 1 : 0} onPress={props.onOpenPalette} /></Tooltip>
          <Tooltip label="Open agent chat" side="bottom"><HeaderButton label="Agent" icon="message" compact={compact} active={props.chatActive ? 1 : 0} onPress={props.onToggleChat} /></Tooltip>
        </Row>
      </Row>
    </Col>
  );
...
export function CompactSurfaceButton(props: any) {
  return (
    <Tooltip label={props.tooltip || props.label} side="bottom">
      <HoverPressable
