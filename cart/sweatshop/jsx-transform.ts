// ── Lightweight JSX -> React.createElement transformer ───────────────
// Ported from love2d/examples/hot-code/src/jsx-transform.ts
...
const TRACKED_TAGS = new Set([
  'Box', 'Text', 'Image', 'Video', 'Pressable', 'ScrollView',
  'TextInput', 'TextEditor', 'Col', 'Row', 'Canvas', 'Graph', 'Native',
  'Terminal',
]);
...
function eat(s: string): boolean {
  if (match(s)) { for (let i = 0; i < s.length; i++) advance(); return true; }
  return false;
}
...
function readIdent(): string {
  let n = '';
  while (pos < src.length && /[a-zA-Z0-9_$.]/.test(peek())) n += advance();
  return n;
}
...
function isJSXStart(): boolean {
  return peek() === '<' && (peekAt(1) === '>' || /[a-zA-Z_$]/.test(peekAt(1)));
}
...
function parseJSX(): string {
  ...
  if (peek() === '>') {
    ...
    if (eat('</')) eat('>');
    return ch.length ? `React.createElement(React.Fragment, null, ${ch.join(', ')})` : 'React.createElement(React.Fragment, null)';
  }
  const tag = readIdent();
...
function parseProps(tag: string, jsxLine: number): string {
  ...
  while (pos < src.length) {
    skipWS();
    if (peek() === '/' || peek() === '>') break;
    if (peek() === '{' && peekAt(1) === '.' && peekAt(2) === '.' && peekAt(3) === '.') {
