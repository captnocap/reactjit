// Probe cart for the Claude backend through the unified worker contract.
//
// Spawns claude_sdk.Session via __worker_start("claude_code", ...),
// streams normalized WorkerEvent rows back. Same shape as the other
// probe carts — different backend tag.
      ...
      <Box style={{ flexDirection: 'column', gap: 4 }}>
        ...
        <Box style={{ paddingTop: 4 }}>
          <span style={{ color: '#888', fontFamily: 'monospace', fontSize: 11 }}>
            worker: {assistant.workerId ?? '(none)'} · backend: claude_code · model: {MODEL}
          </span>
        </Box>
        {assistant.error ? (
          <Box style={{ paddingTop: 4 }}>
