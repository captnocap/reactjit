/*
 * sha512.c — Standalone streaming SHA-512 (FIPS 180-4)
 *
...
void sha512_final(struct sha512_ctx *ctx, uint8_t *hash) {
    ...
    /* Need room for 16 bytes of length at end of block */
    if (ctx->buflen > 112) {
        /* Not enough room — pad this block and compress */
    ...
    /* Append 128-bit length (we only use low 64 bits) */
    memset(ctx->buf + 112, 0, 8);  /* high 64 bits = 0 */
    store_be64(ctx->buf + 120, bits);
