(() => {
  ...
  function loadModel(ggufPath) {
    return callHost("__embed_load_model", 0, ggufPath);
  }
  function freeModel(handle) {
    callHost("__embed_free_model", void 0, handle);
  }
  function nDim(handle) {
    return callHost("__embed_n_dim", 0, handle);
  }
  function embedText(handle, text) {
    const v = callHostJson("__embed_text", null, handle, text);
    return v && v.length > 0 ? v : null;
  }
  function embedBatch(handle, texts) {
    if (texts.length === 0) return [];
    return callHostJson("__embed_batch", [], handle, JSON.stringify(texts));
  }
  function rerank(rerankerGgufPath, query, candidates) {
  ...
  function openStore(slug, dim) {
    return callHost("__embed_store_open", 0, slug, dim);
  }
  function closeStore(handle) {
    callHost("__embed_store_close", void 0, handle);
  }
  function upsert(handle, row) {
    return callHost("__embed_store_upsert", false, handle, JSON.stringify(row));
  }
  function search(handle, qvec, n, sourceType = "") {
    return callHostJson(
  ...
  function ingestStart(rootPath, opts) {
    const ok = callHost(
  ...
  function ingestProgress() {
    return callHostJson("__embed_ingest_progress", EMPTY_PROGRESS);
  }
  function ingestCancel() {
    return callHost("__embed_ingest_cancel", false);
  }

  // runtime/hooks/useEmbed.ts
  function useEmbed(opts) {
    const [ready, setReady] = (0, import_react.useState)(false);
    const [error, setError] = (0, import_react.useState)(null);
    const modelRef = (0, import_react.useRef)(0);
    const storeRef = (0, import_react.useRef)(0);
    const dimRef = (0, import_react.useRef)(0);
    (0, import_react.useEffect)(() => {
      if (!isAvailable()) {
        setError("embed host bindings not registered (framework/v8_bindings_embed.zig)");
        return;
      }
      const m = loadModel(opts.model);
      if (m === 0) {
        setError(`failed to load embedding model: ${opts.model}`);
        return;
      }
      modelRef.current = m;
      dimRef.current = nDim(m);
      const s = openStore(opts.storeSlug, dimRef.current);
      if (s === 0) {
        freeModel(m);
        modelRef.current = 0;
      ...
      setReady(true);
      return () => {
        if (storeRef.current) closeStore(storeRef.current);
        if (modelRef.current) freeModel(modelRef.current);
        storeRef.current = 0;
        modelRef.current = 0;
        setReady(false);
      };
    ...
    const [ingest, setIngest] = (0, import_react.useState)(EMPTY);
    const pollingRef = (0, import_react.useRef)(null);
    function startIngest(rootPath, nWorkers = 4, kind = "code") {
      const ok = ingestStart(rootPath, {
  ...
  // cart/app/isolated_tests/embed_lab/index.tsx
  var DEFAULT_MODEL = "/home/siah/.lmstudio/models/Qwen/Qwen3-Embedding-0.6B-GGUF/Qwen3-Embedding-0.6B-Q8_0.gguf";
  var DEFAULT_SLUG = "qwen3-embedding-0-6b-q8_0";
  var HOME = "/home/siah";
  var KIND_DEFAULT_PATH = {
    ...
    kimi: `${HOME}/.kimi`,
    memory: HOME
    // walker iterates ~/.claude*/projects/<slug>/memory/*.md
  };
  var KIND_LABEL = {
    code: "Code",
    claude: "Claude logs",
  ...
  var KIND_ORDER = ["claude", "claude-overflow", "codex", "kimi", "memory", "code"];
  ...
  function Field({
    ...
    return /* @__PURE__ */ __jsx(Col, { style: { gap: 4 } }, /* @__PURE__ */ __jsx(Text, { style: { fontSize: 12, color: C.dim } }, label), /* @__PURE__ */ __jsx(
      TextInput,
      {
        value,
        onChange,
    ...
    ), hint ? /* @__PURE__ */ __jsx(Text, { style: { fontSize: 11, color: C.dim } }, hint) : null);
  }
  function Button({
    label,
    onPress,
    ...
    return /* @__PURE__ */ __jsx(
      ...
      /* @__PURE__ */ __jsx(Text, { style: { color: fg, fontSize: 13, fontWeight: 600 } }, label)
    );
  }
  ...
  function EmbedLab() {
    const [model, setModel] = (0, import_react2.useState)(DEFAULT_MODEL);
    const [slug, setSlug] = (0, import_react2.useState)(DEFAULT_SLUG);
    const [kind, setKindRaw] = (0, import_react2.useState)("claude");
    const [indexPath, setIndexPath] = (0, import_react2.useState)(KIND_DEFAULT_PATH["claude"]);
    const [agents, setAgents] = (0, import_react2.useState)(String(DEFAULT_WORKERS));
    const [query, setQuery] = (0, import_react2.useState)("");
    const [hits, setHits] = (0, import_react2.useState)([]);
    const [queryMs, setQueryMs] = (0, import_react2.useState)(0);
    const [startedAt, setStartedAt] = (0, import_react2.useState)(0);
    ...
    const phase = ingest.done || !ingest.running && ingest.files_total > 0 ? "done" : ingest.running ? "ingesting" : "idle";
    ...
    return /* @__PURE__ */ __jsx(
      ...
      /* @__PURE__ */ __jsx(Header, { phase, ready: e.ready, error: e.error }),
      /* @__PURE__ */ __jsx(Box, { style: { flexGrow: 1, padding: 24, gap: 16 } }, /* @__PURE__ */ __jsx(
        BuildPanel,
        {
          kind,
          setKind,
      ...
      ), phase !== "idle" ? /* @__PURE__ */ __jsx(IngestPanel, { ingest, startedAt, phase }) : null, phase === "done" ? /* @__PURE__ */ __jsx(
        QueryPanel,
        {
          query,
          setQuery,
  ...
  function Header({ phase, ready, error }) {
    const status = error ? `error: ${error}` : ready ? `model loaded \xB7 phase=${phase}` : "loading model into VRAM\u2026";
    ...
    return /* @__PURE__ */ __jsx(
      ...
      /* @__PURE__ */ __jsx(Col, null, /* @__PURE__ */ __jsx(Text, { style: { color: C.text, fontSize: 16, fontWeight: 600 } }, "Embed Lab"), /* @__PURE__ */ __jsx(Text, { style: { color: C.dim, fontSize: 12 } }, "local Qwen3 + pgvector \xB7 multi-worker ingest in zig")),
      /* @__PURE__ */ __jsx(Box, { style: { flexDirection: "row", alignItems: "center", gap: 8 } }, /* @__PURE__ */ __jsx(Box, { style: { width: 8, height: 8, borderRadius: 4, backgroundColor: color } }), /* @__PURE__ */ __jsx(Text, { style: { color: C.dim, fontSize: 12 } }, status))
    );
  }
  function KindPicker({ kind, setKind }) {
    return /* @__PURE__ */ __jsx(Col, { style: { gap: 4 } }, /* @__PURE__ */ __jsx(Text, { style: { fontSize: 12, color: C.dim } }, "Source kind"), /* @__PURE__ */ __jsx(Row, { style: { gap: 6, flexWrap: "wrap" } }, KIND_ORDER.map((k) => {
      const active = k === kind;
      return /* @__PURE__ */ __jsx(
        Pressable,
        {
        ...
        /* @__PURE__ */ __jsx(Text, { style: { color: active ? "#fff" : C.text, fontSize: 12 } }, KIND_LABEL[k])
      );
    })), /* @__PURE__ */ __jsx(Text, { style: { fontSize: 11, color: C.dim } }, "Picks the parser + walker. ", KIND_LABEL[kind], " \u2192 ", KIND_SOURCE_TYPE[kind], " rows."));
  }
  function BuildPanel({
    kind,
    setKind,
    ...
    return /* @__PURE__ */ __jsx(
      ...
      /* @__PURE__ */ __jsx(Text, { style: { color: C.text, fontSize: 13, fontWeight: 600 } }, "Build an index"),
      /* @__PURE__ */ __jsx(KindPicker, { kind, setKind }),
      /* @__PURE__ */ __jsx(
        Field,
        {
          label: "Model (.gguf)",
      ...
      /* @__PURE__ */ __jsx(Row, { style: { gap: 8 } }, /* @__PURE__ */ __jsx(
        Button,
        {
      ...
      ), ingesting ? /* @__PURE__ */ __jsx(Button, { label: "Cancel", variant: "ghost", onPress: onCancel }) : null)
    );
  }
  ...
  function IngestPanel({
    ...
    const elapsed = startedAt > 0 ? (Date.now() - startedAt) / 1e3 : 0;
    ...
    return /* @__PURE__ */ __jsx(
      ...
      /* @__PURE__ */ __jsx(Row, { style: { justifyContent: "space-between" } }, /* @__PURE__ */ __jsx(Text, { style: { color: C.text, fontSize: 13, fontWeight: 600 } }, phase === "ingesting" ? "Ingesting\u2026" : phase === "done" ? "Done" : ""), /* @__PURE__ */ __jsx(Text, { style: { color: C.dim, fontSize: 12 } }, ingest.files_done, "/", ingest.files_total, " files \xB7 ", ingest.chunks_done, " chunks \xB7", " ", rate.toFixed(1), " chunks/s \xB7 embed-sum ", (ingest.embed_ms_sum / 1e3).toFixed(1), "s")),
      /* @__PURE__ */ __jsx(ProgressBar, { value: pct }),
      ingest.current_file ? /* @__PURE__ */ __jsx(Text, { style: { color: C.dim, fontSize: 11 }, numberOfLines: 1 }, ingest.current_file) : null,
      ingest.error ? /* @__PURE__ */ __jsx(Text, { style: { color: C.err, fontSize: 12 } }, ingest.error) : null,
      ingest.cancelled ? /* @__PURE__ */ __jsx(Text, { style: { color: C.warn, fontSize: 12 } }, "cancelled") : null
    );
  }
  function QueryPanel({
    query,
    ...
    return /* @__PURE__ */ __jsx(
      ...
      /* @__PURE__ */ __jsx(Row, { style: { justifyContent: "space-between" } }, /* @__PURE__ */ __jsx(Text, { style: { color: C.text, fontSize: 13, fontWeight: 600 } }, "Query"), /* @__PURE__ */ __jsx(Text, { style: { color: C.dim, fontSize: 12 } }, hits.length, " hits \xB7 ", queryMs, "ms")),
      /* @__PURE__ */ __jsx(Row, { style: { gap: 8 } }, /* @__PURE__ */ __jsx(Box, { style: { flexGrow: 1 } }, /* @__PURE__ */ __jsx(Field, { label: "", value: query, onChange: setQuery })), /* @__PURE__ */ __jsx(Button, { label: "Run", onPress: onSubmit })),
      /* @__PURE__ */ __jsx(ScrollView, { style: { flexGrow: 1, minHeight: 0 } }, /* @__PURE__ */ __jsx(Col, { style: { gap: 8 } }, hits.map((h2, i) => /* @__PURE__ */ __jsx(HitRow, { key: `${h2.source_id}#${h2.chunk_index}`, hit: h2, rank: i + 1 })), hits.length === 0 ? /* @__PURE__ */ __jsx(Text, { style: { color: C.dim, fontSize: 12 } }, query ? "no results yet" : "type a query\u2026") : null))
    );
  }
  function HitRow({ hit, rank }) {
    return /* @__PURE__ */ __jsx(
      Box,
      {
        style: {
      ...
      /* @__PURE__ */ __jsx(Row, { style: { justifyContent: "space-between" } }, /* @__PURE__ */ __jsx(Text, { style: { color: C.accent, fontSize: 12, fontWeight: 600 } }, rank, ". ", hit.source_id, "#", hit.chunk_index), /* @__PURE__ */ __jsx(Text, { style: { color: C.dim, fontSize: 11 } }, hit.dense_score.toFixed(4))),
      /* @__PURE__ */ __jsx(Text, { style: { color: C.dim, fontSize: 11 }, numberOfLines: 3 }, hit.text_preview)
    );
  }
