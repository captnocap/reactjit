(() => {
  ...
  // cart/app/isolated_tests/tile_drag.tsx
  init_jsx_shim();
  init_ambient();
  init_ambient_primitives();
  ...
  var TILE = 120;
  var GAP = 16;
  var BOARD_W = COLS * TILE + (COLS - 1) * GAP;
  var BOARD_H = ROWS * TILE + (ROWS - 1) * GAP;
  var PALETTE = [
    "#ef4444",
    "#f97316",
    "#eab308",
  ...
  function slotPos(slot2) {
    const col = slot2 % COLS;
    const row = Math.floor(slot2 / COLS);
    return { x: col * (TILE + GAP), y: row * (TILE + GAP) };
  }
  function slotForPoint(localX, localY) {
    const col = Math.max(0, Math.min(COLS - 1, Math.floor(localX / (TILE + GAP) + 1e-4)));
    const row = Math.max(0, Math.min(ROWS - 1, Math.floor(localY / (TILE + GAP) + 1e-4)));
    return row * COLS + col;
  }
  function makeTiles() {
    const out = [];
  ...
  function TileDragCart() {
    const [order, setOrder] = (0, import_react.useState)(() => makeTiles().map((t) => t.id));
    const [tiles] = (0, import_react.useState)(() => makeTiles());
    const [dragId, setDragId] = (0, import_react.useState)(null);
    const [dragDX, setDragDX] = (0, import_react.useState)(0);
    const [dragDY, setDragDY] = (0, import_react.useState)(0);
    const startMouseX = (0, import_react.useRef)(0);
    const startMouseY = (0, import_react.useRef)(0);
    const startSlot = (0, import_react.useRef)(0);
    const orderRef = (0, import_react.useRef)(order);
    const draggingIdRef = (0, import_react.useRef)(null);
    const frameRef = (0, import_react.useRef)(null);
    orderRef.current = order;
    const stopFrame = (0, import_react.useCallback)(() => {
    ...
    const scheduleFrame = (0, import_react.useCallback)((tick2) => {
      const raf = host4.requestAnimationFrame?.bind(host4);
      if (raf) frameRef.current = raf(tick2);
      else frameRef.current = setTimeout(tick2, 16);
    }, []);
    ...
    const beginDrag = (0, import_react.useCallback)((id) => {
      const cur = orderRef.current;
      const slot2 = cur.indexOf(id);
      if (slot2 < 0) return;
      startSlot.current = slot2;
    ...
    (0, import_react.useEffect)(() => () => stopFrame(), [stopFrame]);
    return /* @__PURE__ */ __jsx(Col, { style: { width: "100%", height: "100%", backgroundColor: "#0e0e12", alignItems: "center", justifyContent: "center", gap: 16 } }, /* @__PURE__ */ __jsx(Text, { style: { color: "#e5e7eb", fontSize: 14 } }, "drag tiles to rearrange"), /* @__PURE__ */ __jsx(Text, { style: { color: "#9ca3af", fontSize: 11 } }, "dragId=", dragId == null ? "none" : String(dragId), " dx=", Math.round(dragDX), " dy=", Math.round(dragDY)), /* @__PURE__ */ __jsx(Box, { style: { width: BOARD_W, height: BOARD_H, position: "relative", backgroundColor: "#17171d", borderRadius: 12, borderWidth: 1, borderColor: "#2a2a33" } }, tiles.map((t) => {
      const slot2 = order.indexOf(t.id);
      const pos = slotPos(slot2);
      ...
      return /* @__PURE__ */ __jsx(
        ...
        /* @__PURE__ */ __jsx(Text, { style: { color: "#fff", fontSize: 28, fontWeight: 700 } }, t.label)
      );
    })));
  }
  ...
  if (slot && typeof slot === "object") {
    slot.App = TileDragCart;
  } else {
    g.__lastCartridge = TileDragCart;
  }
})();
