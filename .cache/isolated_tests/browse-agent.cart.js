(() => {
  ...
  init_ffi();
  function useLocalChat(opts) {
    const [phase, setPhase] = (0, import_react.useState)("init");
    const [error, setError] = (0, import_react.useState)(null);
    const [lastStatus, setLastStatus] = (0, import_react.useState)("");
    ...
    const phaseRef = (0, import_react.useRef)("init");
    const askBufRef = (0, import_react.useRef)({
      buf: "",
      resolve: null,
      reject: null
    });
    const toolsRef = (0, import_react.useRef)({});
    const [toolCalls, setToolCalls] = (0, import_react.useState)([]);
    (0, import_react.useEffect)(() => {
      ...
      const ok = callHost("__localai_init", false, cwd, opts.model, sid, nCtx);
      if (!ok) {
      ...
      const id = setInterval(() => {
        setPulse((p) => p + 1);
        while (true) {
          const evt = callHost("__localai_poll", void 0);
          if (!evt) break;
          ...
          if (evt.kind === "system" || evt.kind === "status") {
            ...
            const id2 = String(evt.id || "");
            const name = String(evt.name || "");
            let parsedArgs = {};
            try {
              parsedArgs = evt.args ? JSON.parse(evt.args) : {};
            } catch {
              parsedArgs = { _raw: evt.args };
            }
            setToolCalls((prev) => [...prev, { id: id2, name, args: parsedArgs }]);
            const handler = toolsRef.current[name];
            const replyResult = (val) => {
              const body = (() => {
              ...
              callHost("__localai_send_tool_result", false, id2, body);
            };
            ...
            if (!handler) {
              ...
              try {
                const r2 = handler.execute(parsedArgs);
                if (r2 && typeof r2.then === "function") {
                  r2.then(replyResult, replyError);
                } else {
                  replyResult(r2);
                }
              } catch (e) {
      ...
      return () => {
        clearInterval(id);
        callHost("__localai_close", void 0);
    ...
    (0, import_react.useEffect)(() => {
      ...
      if (!opts.tools || opts.tools.length === 0) {
        callHost("__localai_set_tools", false, "[]");
        return;
      }
      ...
      callHost("__localai_set_tools", false, JSON.stringify(schema));
    }, [opts.tools]);
    function ask(text, _askOpts = {}) {
      ...
      return new Promise((resolve, reject) => {
        ...
        setStreaming("");
        const sent = callHost("__localai_send", false, text);
        if (!sent) {
          askBufRef.current = { buf: "", resolve: null, reject: null };
  ...
  init_ffi();
  var _seq = 1;
  async function browseRequest(cmd) {
    ...
    return new Promise((resolve, reject) => {
      const unsub = subscribe2(`browse:${reqId}`, (payload) => {
        unsub();
        let env;
        try {
      ...
      callHost("__browse_request_async", void 0, body, reqId);
    });
  }
  function setBrowsePort(port) {
    callHost("__browse_set_port", void 0, port);
  }
  function createBrowseTools(_options = {}) {
    return [
      {
        ...
        execute: async ({ url }) => formatPage(await browseRequest({ cmd: "navigate", url }))
      },
      {
        name: "browser_click",
        ...
        execute: async ({ selector }) => browseRequest({ cmd: "click", selector })
      },
      {
        name: "browser_type",
        ...
        execute: async ({ selector, text }) => browseRequest({ cmd: "type_text", selector, text })
      },
      {
      ...
      },
      {
        ...
        execute: async ({ index }) => browseRequest({ cmd: "use_tab", index })
      },
      {
        ...
        execute: async ({ script }) => browseRequest({ cmd: "execute_js", script })
      }
    ];
  }
  function formatPage(r2) {
    if (!r2) return r2;
    const out = { url: r2.url, title: r2.title };
    if (typeof r2.text === "string") {
      const max = 8e3;
      out.text = r2.text.length > max ? r2.text.slice(0, max) + "\n... [truncated]" : r2.text;
    }
    if (Array.isArray(r2.links) && r2.links.length > 0) {
      out.links = r2.links.slice(0, 50);
      if (r2.links.length > 50) out.links_truncated = `${r2.links.length - 50} more`;
    }
    if (Array.isArray(r2.forms) && r2.forms.length > 0) out.forms = r2.forms;
    return out;
  }

  // cart/app/isolated_tests/browse-agent.tsx
  var MODEL = "/home/siah/.lmstudio/models/HauhauCS/Qwen3.5-9B-Uncensored-HauhauCS-Aggressive/Qwen3.5-9B-Uncensored-HauhauCS-Aggressive-Q8_0.gguf";
  var BROWSE_PORT = 7332;
  var RENDER_SRC = `app:browse --port ${BROWSE_PORT} --disposable`;
  var SYSTEM_PRELUDE = [
  ...
  function App() {
    const [prompt, setPrompt] = (0, import_react2.useState)("navigate to https://example.com and tell me the page title");
    const [transcript, setTranscript] = (0, import_react2.useState)([]);
    const [suspended, setSuspended] = (0, import_react2.useState)(false);
    const [firstTurn, setFirstTurn] = (0, import_react2.useState)(true);
    (0, import_react2.useEffect)(() => {
      setBrowsePort(BROWSE_PORT);
    }, []);
    const tools = (0, import_react2.useMemo)(() => createBrowseTools(), []);
    ...
    (0, import_react2.useEffect)(() => {
      ...
      setTranscript((tr) => {
        ...
        const news = chat.toolCalls.map((tc) => ({ role: "tool", text: `${tc.name}(${JSON.stringify(tc.args)})` })).filter((t) => !have.has(t.text));
...
User: ${user}` : user;
    ...
    return /* @__PURE__ */ __jsx(Box, { style: { width: "100%", height: "100%", backgroundColor: "#0f0f1a" } }, /* @__PURE__ */ __jsx(Row, { style: { paddingTop: 8, paddingBottom: 8, paddingLeft: 12, paddingRight: 12, gap: 12, backgroundColor: "#181825", alignItems: "center" } }, /* @__PURE__ */ __jsx(Text, { style: { color: "#cdd6f4", fontSize: 13, fontWeight: "bold" } }, "browse-agent"), /* @__PURE__ */ __jsx(Text, { style: { color: "#6c7086", fontSize: 11 } }, `local llama: ${chat.phase}${chat.lastStatus ? " \xB7 " + chat.lastStatus : ""}`)), /* @__PURE__ */ __jsx(Row, { style: { flexGrow: 1, gap: 4, padding: 4 } }, /* @__PURE__ */ __jsx(Col, { style: { flexGrow: 1, flexBasis: 0, gap: 6, backgroundColor: "#11111b", borderRadius: 4, padding: 8 } }, /* @__PURE__ */ __jsx(Text, { style: { color: "#89b4fa", fontSize: 11, fontWeight: "bold" } }, "chat"), /* @__PURE__ */ __jsx(ScrollView, { style: { flexGrow: 1, backgroundColor: "#1e1e2e", borderRadius: 3, padding: 6 } }, transcript.map((t, i) => /* @__PURE__ */ __jsx(Row, { key: i, style: { paddingTop: 4, paddingBottom: 4, gap: 6, alignItems: "flex-start" } }, /* @__PURE__ */ __jsx(Text, { style: {
      color: t.role === "user" ? "#f9e2af" : t.role === "assistant" ? "#a6e3a1" : t.role === "tool" ? "#74c7ec" : "#f38ba8",
      fontSize: 11,
      fontWeight: "bold",
      minWidth: 70
    } }, t.role), /* @__PURE__ */ __jsx(Text, { style: { color: "#cdd6f4", fontSize: 11, flexGrow: 1 } }, t.text))), chat.streaming ? /* @__PURE__ */ __jsx(Row, { style: { paddingTop: 4, paddingBottom: 4, gap: 6, alignItems: "flex-start" } }, /* @__PURE__ */ __jsx(Text, { style: { color: "#a6e3a1", fontSize: 11, fontWeight: "bold", minWidth: 70 } }, "assistant\u2026"), /* @__PURE__ */ __jsx(Text, { style: { color: "#bac2de", fontSize: 11, flexGrow: 1 } }, chat.streaming)) : null), /* @__PURE__ */ __jsx(
      TextInput,
      {
        value: prompt,
    ...
    ), /* @__PURE__ */ __jsx(Row, { style: { gap: 4 } }, /* @__PURE__ */ __jsx(
      Pressable,
      {
        onPress: send,
        style: {
      ...
      /* @__PURE__ */ __jsx(Text, { style: { color: chat.phase === "generating" ? "#9399b2" : "#0f0f1a", fontSize: 11, fontWeight: "bold" } }, chat.phase === "generating" ? "generating\u2026" : "send")
    ), /* @__PURE__ */ __jsx(
      Pressable,
      {
        onPress: () => {
      ...
      /* @__PURE__ */ __jsx(Text, { style: { color: "#cdd6f4", fontSize: 11 } }, "clear")
    ))), /* @__PURE__ */ __jsx(Col, { style: {
      flexGrow: 1,
      flexBasis: 0,
      borderWidth: 1,
      borderColor: "#2a2a4a",
      ...
      overflow: "hidden",
      backgroundColor: "#000"
    } }, /* @__PURE__ */ __jsx(Row, { style: { paddingTop: 4, paddingBottom: 4, paddingLeft: 8, paddingRight: 8, backgroundColor: "#181825", alignItems: "center", gap: 8 } }, /* @__PURE__ */ __jsx(Text, { style: { color: "#89b4fa", fontSize: 11, fontWeight: "bold", flexGrow: 1 } }, "firefox (browse stealth \xB7 port ", BROWSE_PORT, ")"), /* @__PURE__ */ __jsx(
      Pressable,
      {
      ...
      /* @__PURE__ */ __jsx(Text, { style: { color: suspended ? "#0f0f1a" : "#cdd6f4", fontSize: 10 } }, suspended ? "resume" : "suspend")
    )), /* @__PURE__ */ __jsx(
      Render,
      {
