...
function SDL2Init.run(config)
  ...
  -- Adaptive vsync (-1): swap immediately when a frame misses the deadline,
  -- only wait for vsync when the frame finishes early. This prevents hard vsync
  -- from halving FPS when a frame barely overruns (e.g. 33fps on a 240Hz display
  -- because every 4.17ms deadline miss waits for the next interval).
  -- Falls back to hard vsync (1) if adaptive isn't supported, then no vsync (0).
  local vsyncOk = sdl.SDL_GL_SetSwapInterval(-1) == 0
  if not vsyncOk then
    vsyncOk = sdl.SDL_GL_SetSwapInterval(1) == 0
  end
  ...
  while running do
    ...
    if #commands > 0 then
      ...
      if treeDirty then
        ...
        if not tOk then
          ...
          treeDirty = false  -- don't run syncs on failed apply
        else
    ...
    -- ---- Capabilities sync (runs every frame — includes per-instance tick()) ----
    Shim.setDelta(dt)
    capabilities.syncWithTree(tree.getNodes(), pushEvent, dt)

    -- ---- Generative effects sync + update + render ----
    if effectsmod then
      if treeDirty then effectsmod.syncWithTree(tree.getNodes()) end
    ...
    if Videos then
      if treeDirty then Videos.syncWithTree(tree.getNodes()) end
      Videos.renderAll()
    ...
    for _, win in ipairs(allWindows) do
      ...
      -- Secondary windows swap immediately (interval 0) to avoid multiple
      -- vsync waits per frame — each one blocks for a full interval.
      if not win.isMain then sdl.SDL_GL_SetSwapInterval(0) end
