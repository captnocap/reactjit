...
---@param port integer
---@param path string
---@param opts table|nil  Options: { socket = pre-connected TCP socket }
---@return wsclient
function _M.new(host, port, path, opts)
    opts = opts or {}
    local m = {
        url = {
            host = host,
        ...
        socket = opts.socket or socket.tcp(),
    }
    m.socket:settimeout(0)

    if opts.socket then
        -- Pre-connected socket (e.g., via SOCKS5 tunnel).
        -- Send WebSocket upgrade immediately, skip TCPOPENING.
        local url = m.url
        m.socket:send(
